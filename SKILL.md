...
summary: Diagnose CPU, memory, allocation, GC, contention, startup, I/O, native interop, UI, rendering, GPU, and production performance problems in .NET applications across macOS, Windows, and Linux.
description: Evidence-first performance engineering workflow for coding agents combining portable .NET diagnostics with platform-native CPU, memory, scheduler, I/O, graphics, GPU, deployment, container, and statistical profiling procedures.
---

# .NET Performance Engineering

Use this skill when investigating, explaining, fixing, or validating the performance of a .NET application, including ASP.NET Core, services, console applications, Avalonia, WPF, WinUI, MAUI, Skia, WebGPU, game/graphics engines, NativeAOT, and native-interoperability workloads.
...
- Profile an optimized build, normally `Release`, and the real deployment form.
- Record commit, SDK/runtime, OS, architecture, hardware, power mode, workload, profiler versions, and exact commands.
...
- Do not claim a fix from one average, one trace, one frame capture, or the fastest benchmark run.
...
- Protect secrets and personal data in dumps, traces, logs, SQL spans, and packet captures.
...
4. **Evidence** — counters, traces, captures, dumps, and distributions.
...
- Microbenchmarks, application workloads, statistics, CI thresholds, before/after reports:
...
9. For production-only incidents, use bounded/triggered collection and record cgroup/host constraints.
10. Implement only after evidence supports the dominant cause.
11. Repeat the identical workload and publish median/spread/tail values plus raw artifacts.
...
- Keep large trace/capture/dump binaries out of Git unless repository policy explicitly stores them.
